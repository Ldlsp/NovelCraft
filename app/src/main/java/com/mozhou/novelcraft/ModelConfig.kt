...
class ModelPreferences(context: Context) {
    ...
    fun load(): ModelConfig = ModelConfig(
        ...
        protocol = cloudProtocol(preferences.getString("protocol", "openai").orEmpty()),
        ...
        imageProtocol = preferences.getString("image_protocol", "openai").orEmpty(),
        ...
        reviewerProtocol = preferences.getString("reviewer_protocol", "").orEmpty().takeIf { it.isBlank() } ?: cloudProtocol(preferences.getString("reviewer_protocol", "").orEmpty()),
    )
    ...
    fun save(config: ModelConfig) {
        preferences.edit()
            ...
            .putString("image_protocol", config.imageProtocol)
            ...
            .putString("reviewer_protocol", config.reviewerProtocol)
            .apply()
    }

    private fun cloudProtocol(value: String): String = value.takeIf { it in setOf("openai", "anthropic", "gemini", "azure") } ?: "openai"
}
            ...
            retryAiRequest(request) {
                ...
                try {
                    ...
                    if (config.imageProtocol == "gemini") {
                        ...
                        return@retryAiRequest android.util.Base64.decode(inline.getString("data"), android.util.Base64.DEFAULT)
                    }
            ...
            retryAiRequest(request, canRetry = { !receivedStreamDelta }) {
                ...
                try {
                    ...
                    if (status !in 200..299) {
                        ...
                        throw AiHttpException(status, "HTTP $status: ${errorBody.take(180)}")
                    }
